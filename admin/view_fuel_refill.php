...
<div class="dropdown-menu" aria-labelledby="manageFuelDropdown">
    ...
    <h2 class="text-center">View Fuel Refill</h2>
    <div class="card mt-4">
        <div class="card-header">
            <h4 class="mb-0">Fuel Refill Records</h4>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT vehicle.Model, fuel_refills.RefillDate, fuel_refills.FuelQuantity, fuel_refills.Cost 
                                FROM fuel_refills 
                                INNER JOIN vehicle ON fuel_refills.VehicleID = vehicle.VehicleID";
                        ...
                        if (!$result) {
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr>
                                            <td>" . $row["Model"] . "</td>
                                            <td>" . $row["RefillDate"] . "</td>
                                            <td>" . $row["FuelQuantity"] . "</td>
                                            <td>" . $row["Cost"] . "</td>
                                        </tr>";
                                }
                            } else {
                                echo "<tr><td colspan='4'>No fuel refill records found</td></tr>";
                            }
                        }
                        $conn->close();
                        ?>
